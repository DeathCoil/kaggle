...
def extract_match_features(match, time_point=None):
    ...
    items = np.arange(11, 47).tolist()
    items += [2, 4, 5, 6,
              50, 56, 57, 59, 60, 61, 63, 65, 69, 71, 73, 75, 77,
              84, 86, 88, 92, 94, 151, 164, 178,
              180, 181, 182, 187, 188, 212, 214, 215, 216, 218]

    extract_items_count += list(zip(items, map(str, items)))
    extract_items_time += list(zip(items, map(str, items)))
    ...
    if picks_bans is not None:
        for action in picks_bans:
            if not action["is_pick"]:
                if action["team"] == 1:
                    bans += "d" + str(d_index) + "_ban_" + str(action["hero_id"]) + "."
                    d_index += 1
                else:
                    bans += "r" + str(d_index) + "_ban_" + str(action["hero_id"]) + "."
                    r_index += 1
            feats += [("bans", bans)]
    else:
        feats += [("bans", "")]
